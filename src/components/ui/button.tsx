...
const buttonVariants = cva(
  "inline-flex items-center  justify-center px-1 md:px-3 whitespace-nowrap rounded-md text-sm font-medium transition-all duration-300  ring-offset-background transition-all focus-visible:outline-none focus-visible:hidden focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50",
  {
    variants: {
      color: {
        default:
          "bg-default text-default-foreground hover:bg-default hover:ring-default dark:disabled:bg-default-500",
      ...
      variant: {
        default: "",
        outline:
          "border border-secondary text-default bg-transparent hover:bg-default hover:text-default-foreground hover:ring-0 hover:ring-transparent",
        ...
        shadow: "shadow-md",
        active_outline: "border border-primary text-primary bg-default",
        inactive_outline:
          "border border-borderad text- bg-default hover:bg-secondary  hover:ring-0 hover:ring-transparent",
      },
      shadow: {
        sm: "shadow",
        md: "shadow-md",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    { className, variant, size, color, fullWidth, shadow, rounded, asChild = false, ...props },
    ref
  ) => {
    ...
    return (
      <Comp
        className={cn(
          buttonVariants({ variant, size, fullWidth, rounded, color, shadow, className })
        )}
        ref={ref}
        {...props}
      />
