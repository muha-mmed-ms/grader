...
export default function DifficultyMarksTable({ data }: { data?: DifficultyData[] }) {
  return (
    <Card className="w-full mx-auto shadow-md">
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-gray-900">
          Difficulty wise Marks Distribution
        </CardTitle>
      </CardHeader>
      <CardContent>
            ...
            <TableBody>
              {data?.map((row, index) => (
                <TableRow key={index} className="border-b hover:bg-gray-50">
                  ...
                  <TableCell className="py-4 text-gray-900">{row.difficultyLevel}</TableCell>
